...
- Task 8 启动层需要集中组装配置、人脸库、后端、摄像头、识别流水线、工作线程、控制器和三个窗口，避免任何界面类自行创建硬件或模型。
- 当前 README 仍停留在 Phase 4 的 28 项测试与“Phase 5 待实现”描述，Task 8 完成后必须同步为可运行桌面应用，并明确 Fake 演示、ONNX 实机、USB 摄像头检查和常见错误。
- 两个 ONNX 适配器实际统一位于 inference/onnx_backend.py；检测器接收模型路径与 detection_threshold，特征提取器接收模型路径，启动层无需更改推理模块。
